...
func GenerateNewPassword() *Password {
	return &Password{value: utils.RandomPassword()}
}
...
func HashedPasswordFrom(hashedValue string) *HashedPassword {
	...
	if len(cost) == 1 {
		if cost[0] < bcrypt.MinCost {
			return nil, fmt.Errorf(
				"cost too low; must be at least %d",
				bcrypt.MinCost+4,
			)
		}
...
func GenerateTestHashPassword() *HashedPassword {
	return &HashedPassword{value: utils.RandomPassword()}
}
